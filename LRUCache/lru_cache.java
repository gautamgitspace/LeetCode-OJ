class LRUCache {
  ...
  public int get(int key) {
    ...
    // else Item exists and has been accessed.
  ...
  public void put(int key, int value) {
    // fetch node at key from hash table
    ListNode node = hashtable.get(key);
    ...
    if (node == null) {
      ...
      if (totalItemsInCache > maxCapacity) removeLRUEntry();
    } else {
      // item is found in the cache, we update it
      // and we move it to the head of the list
      node.value = value;
      moveToHead(node);
    }
  }
  ...
  private void addToFront(ListNode node) {
    ...
    node.prev = head;       // link node backwards to head
    node.next = head.next;  // link nodes forward to head's next
    head.next.prev = node;  // link head's next backwards to node
    head.next = node;       // finally update head's next to be this node
  }
